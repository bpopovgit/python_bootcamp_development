...
def check_if_user_lost(user_score_, computer_score_):
...
while not is_game_over:
    ...
    while player_choice == 'y':
        ...
        user_score = calculate_player_score_after_player_choice(updated_user_card_list, user_score)
        print(f"Your cards: {updated_user_card_list},"
              f" current score: {user_score}")
        print(f"Computer's first card: {computer_cards[0]}")
        if check_if_user_lost(user_score, computer_score):
            is_game_over = True
